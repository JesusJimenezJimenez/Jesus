<?php
 ...
 // Tabla con instancias
  $cols = array('*','-fecha','Fecha de registro' => '@{substr("@[fecha]", 0, 10)}@', '-contraseña', 'Alguna Accion'=>'<a href="?Suprimir=@[0]">Suprimir</a>');
  $join = array('depende'=>$liga);
  $pie  = '<th colspan="@[numCols]">Total de usuarios: @[numReg]</th>';
  HTML::tabla($liga, 'Instancias de '.$tabla, $cols, true, $join, $pie);
  
  // Formulario para crear nuevas instancias
  $props  = array('form'=>'method="POST" action="?accion=insertar"','contraseña' => '<input type="password" name="contraseña" id="contraseña" />',
  ...
  $props  = array('form'=>array('method'=>'POST', 'action'=>'?accion=modificar'), 'prefid'=>'algo','contraseña' => '<input type="password" name="contraseña" id="contraseña" />',
